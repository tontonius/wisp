...
Matches the same rules as internal GPU selection (`simulation`, `auto` threshold, `gpu.forceCpuFallback`). Used for warning logic (for example `onParticleDeath` on GPU).
