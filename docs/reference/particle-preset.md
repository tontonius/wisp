...
| `subEmitters` | `{ onDeath?: string }` | `undefined` | CPU-only child effect hooks. `onDeath` spawns a named effect from each CPU particle death (when spawned through `ParticleWorld`). |
